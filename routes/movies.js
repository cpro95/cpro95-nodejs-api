...
router.get('/', (req, res, next) => {
    ...
    if (Object.keys(req.query).length === 0) {
        ...
        query.map((item) => {
            ...
            if (item === 'limit') {
                if (sql === '') {
                    sql += `select idMovie, c00, c08, c20, premiered, rating from movie_view order by premiered desc limit ${req.query[item]}`;
                } else {
            ...
            if (item === 'offset') {
                if (sql === '') {
                    sql += `select idMovie, c00, c08, c20, premiered, rating from movie_view order by premiered desc offset ${req.query[item]}`;
                } else {
